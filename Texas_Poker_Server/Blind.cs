        ...
        public Blind()
        {
            ...
            for (int i = Blind_position; i < Now_sit.Length + Blind_position; i++)
            {
                ...
                if (Now_sit[j] == 1)
                {
                    ...
                    if (count == 0 )
                    {
                        data = Encoding.ASCII.GetBytes("Small_Blind" + " " + Big_Blind.ToString() + " " + j.ToString());
                        Player_Raise_Money[j] = Big_Blind / 2;
                        count++;
                    }
                    else if (count == 1)
                    {
                        data = Encoding.ASCII.GetBytes("Big_Blind" + " " + Big_Blind.ToString() + " " + j.ToString());
                        Player_Raise_Money[j] = Big_Blind;
                        Raise_position =( j % Now_sit.Length)+1;
                        count++;
                    }
                    else
                        data = Encoding.ASCII.GetBytes("Normal" + " " + Big_Blind.ToString() + " " + j.ToString());
                    sClient[j].Send(data);
